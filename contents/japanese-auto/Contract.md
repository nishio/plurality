...
&nbsp;&nbsp;&nbsp;&nbsp;在多元社會中，大多數大規模合作是通過將資產集中到“企業”實體中進行的，包括合夥企業、非營利組織、宗教和公民組織，當然還有公司。雖然很明顯並非所有公司都是以正式意義上的公司，但它們與公司一樣，都有一個合同安排的法律基礎，該合同安排規定了在共同的目標下共享資產（實物、知識、人力和財務）的方式。即使是最簡單、最常見和最小規模的合同，如租賃協議，也涉及到人們之間的資產共享。
...
&nbsp;&nbsp;&nbsp;&nbsp;如同凱特·克勞福德（Kate Crawford）在她精美繪製的《AI 地圖集》中所強調的，數位世界是建立在物理世界之上的：電腦電路是由稀有金屬製成，而這些金屬的開採帶來了各種社會挑戰；數據中心的運作方式與發電廠非常相似，而且通常是與發電廠共同設置；數據是由像瑪麗·格雷（Mary Gray）和西達斯·蘇里（Siddarth Suri）所記錄的“幽靈工人”等人創造的[^Phys]。因此，對於數位領域的嚴肅描述必須涉及到實際的財產關係。然而，從這些物理基礎中產生的關鍵資產是數位原生的抽象，也是線上生活的關鍵組成部分。
...
最接近開放標準的資產共享框架存在於存儲中，通過星際文件系統（IPFS）明確地模仿Lick的願景並由Juan Benet和他的Protocol Labs（PL）開創。這個開放協議允許世界各地的計算機以點對點的方式互相提供存儲，以碎片化、加密和分佈式的方式，有助於確保用戶的冗余性、韌性和數據的保密性/完整性，對於為系統提供存儲的人來說，成本也是合理的。建立在該協議上的知名服務包括台灣數位事務部和其他面臨強大對手的政府使用，這些對手可能對更集中的服務提供商擁有影響力，以確保其數據的持久性，以及由PL的Filecoin系統創建的存儲市場，以允許基於IPFS的存儲進行商業交易。
...
最初，推動公司追求這個機會的微軟團隊等“雲時代”的先驅者認為，雲端帶來的許多收益來自於跨租戶和應用程序之間更有效的資源共享，以確保充分利用。然而，在實際應用中，雲端帶來的大部分收益來自於與豐富的電源源和高效維護的數據中心共同設置的物理成本節省，而不是來自於有意義的跨租戶資源共享，因為很少有雲端服務提供商有效地促進了這種市場，也很少有客戶找到了使資源共享對他們起作用的方法。
...
&nbsp;&nbsp;&nbsp;&nbsp;我們可以從這些失敗中獲得哪些教訓，以了解更有效地共享數字資產的障礙？從數據共享最為失敗且在數據共享問題上遇到最大困難的事實來看，一個自然的假設是相關問題可能是許多這些問題的核心所在。畢竟，所有這些領域都會出現相關挑戰。 IPFS的結構和面臨的挑戰很大程度上與在允許存儲遠離尋求保持此隱私的個人或組織的同時維護數據隱私有關。雲服務提供商的一個核心優勢是他們在允許客戶在設備之間共享數據並對其進行大規模計算的同時，維護客戶數據的安全和隱私。
...
&nbsp;&nbsp;&nbsp;&nbsp;我们在协会章节中已经讨论过，但在这里回顾它们的相关性是值得的。安全多方计算（SMPC）和同态加密允许多个参与方一起进行计算，并创建一个集体输出，而不会向其他人透露输入。最简单的示例包括计算平均工资或计算选举中的投票数，但越来越多的复杂应用正在逐渐实现，例如训练或微调GFM。这些更有雄心的应用帮助创建了“联邦学习”和“数据联邦”领域，它们允许这些雄心勃勃的应用所需的计算在个人或组织计算机的分布式网络上本地执行，模型的输入在各方之间安全地传递，而底层训练数据永远不会离开通信各方的机器或服务器。与OpenMined等开源工具提供商合作，像联合国这样的国际组织越来越多地建立了利用这些工具的数据协作实验平台。与这种分布式方法相反，还可以使用专门的“保密计算机”，可以验证其执行特定计算，但没有人可以访问其中间输出。然而，由于这些机器昂贵且只由有限的一些公司生产，它们更适合由可信的中央实体控制，而不是分散的协作。
 
&nbsp;&nbsp;&nbsp;&nbsp;这些方法可以帮助实现协作，而不会在协作者之间传递不必要的信息，但是还需要其他工具来处理协作产生的所需输出（例如统计数据或模型）中包含的信息。模型可能会泄露输入信息（例如，模型会重现某个特定人的详细病史），或者相反，可能会掩盖信息的来源（例如，违反许可证，在没有归属的情况下重现输入的创造性文本）。这两种情况都对数据协作构成重大障碍，因为协作者通常希望对其数据的使用拥有自主权。
 
&nbsp;&nbsp;&nbsp;&nbsp;这些挑战的解决工具更多是统计学的而不是密码学的。差分隐私限制了从一组输出数据中猜测输入数据的程度，使用“隐私预算”来确保一起的披露不可靠地揭示输入。水印可以在内容中创建“签名”，显示其来源的方式很难被擦除、忽视甚至在某些情况下被检测到。"影响函数"追踪特定数据集在生成模型输出中所起的作用，允许至少部分归因于一个本质上是"黑盒子"模型的输出。
  
&nbsp;&nbsp;&nbsp;&nbsp;所有这些技术在速度、规模和功能方面都有些落后于GFMs的发展。例如，差分隐私主要关注事实的统计可恢复性，而GFMs通常能够像侦探一样进行“推理”，例如从一系列只有松散相关的事实中推断出某人的第一所学校、友谊等。同样，追踪神经网络中的来源和价值归因方面进展甚微。利用这些模型解决这些技术挑战，并在模型进一步发展时推导出数据保护和归因的技术标准定义，将对于使数据协作可持续至关重要。
...
社会漏洞：即使数据不直接来自社交互动，它们几乎总是具有社会影响。例如，由于亲属之间共享的遗传结构，对人口的1%统计样本可以通过遗传资料识别任何个体，因此保护遗传隐私是一项深具社会意义的任务。
...
一旦它们得到充分发展和传播，数据协作工具、组织和实践可能会变得足够熟悉，以至于能够像"财产权"一样深入地被编码到常识和法律实践中，尽管正如我们所指出的，它们几乎肯定需要采取与私有土地所有权或联合股份公司组织的标准模式不同的形式。正如我们所指出的，它们将需要包含更多的技术和密码学元素，不同类型的社会组织，更加强调集体治理和受托责任的职责和规范，或者通过法律保护免受成员单方面披露的规定（类似于禁止单方面破坏工会的规定）。这些可能会形成数字世界中的未来版本的"财产"，但更加适应数据的复数特性。
...
&nbsp;&nbsp;&nbsp;&nbsp;兩個自然的例子是電磁譜和命名空間。這兩個領域都是傳統財產權正在迅速重新想像的領域。
