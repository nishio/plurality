...
最も基本的な権利である生命、人格、市民権と同様に、ネットワーク社会のための最も基本的なプロトコルは、参加者のアイデンティティを確立し保護するものです。これらの権利やサービスを確保するためには、誰が何に権利を持つかの定義がなければなりません。十分に安全なアイデンティティの基盤がなければ、例えば投票システムは最も多くの偽の資格情報を提供できる人によって支配され、富豪政治に堕落してしまいます。1993年の有名なニューヨーカーの漫画「インターネットでは、誰もあなたが犬だと知りません」は非常に有名で、[ウィキペディアのページ](https://en.wikipedia.org/wiki/On_the_Internet,_nobody_knows_you%27re_a_dog)まで存在します。これが真実である限り、オンライン民主主義の試みは、文字通り犬の手に落ちることが予想されます。これは、疑名性や匿名性に大いに依存している多くの「Web3」コミュニティでドラマ化されており、物理的および金融的なリソースにアクセスできる者の利益によってしばしば支配されています。

したがって、アイデンティティシステムはデジタルライフにおいて中心的な役割を果たし、ほとんどのオンライン活動へのアクセスを制御しています。ソーシャルメディアアカウント、電子商取引、政府サービス、雇用、定期購読などです。各システムが提供できるものは、ユーザーのアイデンティティをどれだけ豊かに確立できるかに密接に依存しています。たとえば、ユーザーが人間であることしか判断できないシステムは、このオファーに既にサインアップしていないことを確認しない限り、無料の特典を提供することはできません。ユーザーが一意であることを判断できるが、それ以外の情報を提供できないシステムは、法的かつ実用的に地球上のすべての人に提供できるサービスのみを提供することができます。オンラインでの攻撃の容易さを考慮すると、オンライン上には個人に関する情報のみが安全に存在できます。
...
この章では、既存のデジタルアイデンティティシステムの運用と、確立と保護の二重の要請をどのように航行するかについて議論します。次に、これらの問題に対処するために世界中で行われているいくつかの重要で限定された取り組みについて議論します。その後、この重要な作業をより野心的に拡大し、より⿻未来を実現する方法を示します。最後に、アイデンティティの基本的な役割のため、他の基本的なプロトコルや権利、特に次の章で焦点を当てる結社権利とどのようにつながり、絡み合うかを強調します。
...
このスペクトルからは、より小さく、より多様で、より地域的なアイデンティティシステムが、デジタルネイティブなコンテキストとより伝統的なコンテキストの両方で、まったく異なる方向に存在しています。
...
一方、Worldcoinのようにプライバシーが保護されている場合、バイオメトリクスを使用してアカウントを初期化することで、システムはアカウントの盗難や売却のリスクにさらされます。人々がアクセスしようとするほとんどのサービスは、単にユニークな人間であることを証明する以上の情報が必要です（たとえば、特定の名前を持っていること、認識された政府から発行されたID番号を持っていること、ある国の市民であること、おそらく会社での教育や雇用の資格などの他の属性を持っていること）。このようなプライバシーの極端な保護は、システムのほとんどの有用性を損ないます。さらに、このようなシステムはバイオメトリクスシステムの技術的な性能に大きな負担をかけます。将来的に、AIシステムと高度な印刷技術によって虹彩が偽装される可能性がある場合、そのようなシステムは極端な「単一障害点」にさらされる可能性があります。要するに、包括性とシンプルさの重要な能力を持ちながらも、バイオメトリクスシステムは、豊かさとセキュリティが必要なアイデンティティの確立と保護を達成するにはあまりにも簡素すぎます。
...
この看板のように、中央集権的な監視なしで安全な確立と強力なアイデンティティ保護を確保する方法はあるのでしょうか？自然な答えは、「失われた道」で説明した⿻の伝統に基づいています：アイデンティティの⿻、交差する性質とネットワークアーキテクチャの可能性を高く評価すること。パケットスイッチングが分散化とパフォーマンスを調和させ、ハイパーテキストが速度とテキストを通じた多様な経路を調和させたように、実験と標準の適切な組み合わせによって、アイデンティティへの交差的アプローチが確立と保護の目標を調和させる可能性がますます高まっているようです。
...
上記の利点は、上記で説明したトレードオフと比較すると注目に値します。 しかし、基本的には、「The Lost Dao」で議論した利点のかなり単純な拡張です。ネットワーク化された構造が一般的に中央集権化された構造よりも持つ利点、通信ネットワークのパケットスイッチングアーキテクチャへの移行を促した利点と同様です。 これは、Trust over IP Foundationなどの将来的にこのような未来を実現しようとする主要な組織の中には、インターネットプロトコルの歴史と密接な類推を行うものがあるためです。 もちろん、そのような「交差点」システムを機能させるためには、多くの技術的および社会的な課題があります。
...
複数主義者の考え方と社会的距離を管理する方法は何ですか？私たちは将来の章でGFMsの潜在的な役割に戻ります。代わりに、堅牢なネットワークベースのアプローチに焦点を当てると、2つの自然な戦略が対応します。これは、「分散化」（または「ポリセントリズム」とも呼ばれ、私たちが使用する）と呼ばれるもので、重要なサイズの検証者が多数存在しますが、圧倒的な複雑さを生み出すほど多くはありません。また、「分配」と呼ばれるものもあり、こちらでは大規模な検証者が少数存在し、代わりに社会的距離を横断するために推移的な信頼を使用します。これらすべての可能性を考慮する際に心に留めておくと役立つ基本的なヒューリスティックスは、「ダンバー数」です。これは、人類学者のロビン・ダンバーが、重要な情報技術がない状況で人々が維持できる人数（通常約150人）です。正確な数値は何であれ、ほとんどの人が数百の関係や評価などを重要な技術の支援なしで管理できないことは明らかです。

ポリセントリックアプローチは、プレイヤーの数を制限することでこの問題を管理しようとします。これは明らかに多元主義を制限しますが、参加者が合理的な所属の多様性を維持すれば、それほど大きな問題ではありません。たとえば、100億人の人口がいて、それぞれの人が潜在的に検証機関（政府、教会、雇用主など）と100の関係を維持しているとします。検証がうまく機能するためには、出会った2人が少なくとも5つの重複するメンバーシップを共有する必要があるとします。メンバーシップがランダムに分布していると仮定すると、300の検証者が共存しても、ランダムな2人の間で検証が失敗する確率が数百万分の1である可能性があります。もちろん、出会う個人はめったにランダムではなく、所属もランダムに形成されるわけではなく、特にランダムに出会った人々の間では5つの重複するメンバーシップが絶対に必要とされることは少ないでしょう。これらすべてが、そのような環境で多くの検証者が繁栄する可能性を示唆しています。
...
さらに、分散化された戦略と分散戦略は互いを大幅に増幅させることができます。たとえば、属性の発行者が10万人いるかもしれないという私たちの提案を考えてみてください。100億人の世界では、平均してそれぞれが10万人の参加者との関係を管理する必要があります。彼らが他の発行者とも同様の数の関係を管理できる場合、すべての発行者が他のすべての発行者と直接的な関係を持つことになります。2つの分離度ははるかに多くのことを成し遂げることができ、他の発行者から属性を活用して検証を行うことができる論理の下で、何百万もの発行組織が繁栄することができます。したがって、推移的信頼と多中心主義の組み合わせは、私たちが以下で議論するGFMsの魔法なしでも、高度で多様であり、したがって機能的でプライベートなアイデンティティの景観を容易に実現することができます。
...
その後の鍵となる質問は、そのような⿻と社会的検証のプロセスが、アイデンティティの保護を弱体化させる可能性があるかどうかです。 結局のところ、私たちがこのような機能不全のアイデンティティの風景を持っている理由の中核的な部分は、リベラルな民主主義体制がアイデンティティシステムの作成に抵抗してきたことです。 もし私たちがより良い代替案を構築するのであれば、それらがこの次元において何よりもより良いものであることを確認する必要があります。 しかし、それを行うには、⿻の観点から「プライバシー」と「コントロール」が具体的に何を意味するのかをより深く掘り下げる必要があります。
